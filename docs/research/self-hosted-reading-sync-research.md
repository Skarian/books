...
Family/multi-user support is viable only with the constraints in `family-multi-user-admin-plan.md`: shared books are fine, but OPDS credentials, KOSync progress, setup pages, and upload permissions must be per-user.

## Hard Constraints

- No paid service in the core path.
- No non-self-hosted progress service or hosted library custody.
- Runtime secrets, books, and sync databases stay outside git.
...
- Multi-user mode must never share KOSync credentials between people.
...
| Android | Readest | OPDS for books; KOSync for progress | Accept |
| iPad | Readest | OPDS for books; KOSync for progress | Accept |
| MacBook | Readest | OPDS for books; KOSync for progress | Accept |
| Windows PC | Readest | OPDS for books; KOSync for progress | Accept |
...
KOSync is the canonical cross-app progress lane. Readest uses OPDS for book
downloads and KOReader Sync for progress. WebDAV is not part of the default
architecture.

## Why This Works

Readest's current docs describe OPDS catalog support and a direct
Readest-to-KOReader Sync Server mode. OPDS downloads go into the Readest
library. KOReader Sync is progress-only, needs Readest 0.10.1+, and uses the
same server URL, username, password, and file-content checksum approach as
KOReader binary matching.

Readest source at `4c39d769` confirms the important combined path: an OPDS
download imports as a normal local-library book, Readest computes `book.hash`
from the file content, and KOReader Sync uses that same `book.hash` as the
document id. WebDAV is not involved in that path.
...
It is also very small. It stores progress in Redis by user and document hash,
uses MD5-derived user keys, and has no admin lifecycle API. Registration is
gated by `ENABLE_USER_REGISTRATION`, so onboarding should enable registration
only long enough to create users, then disable it. Password rotation, disable,
and purge must be implemented by the repo helper against a pinned server/data
layout and verified by probes.

The server is last-write-wins. It does not protect against a stale client
moving progress backward. For this project that means one KOSync account per
person and device tests that include “later lower progress wins” so we know the
actual conflict behavior.
...
Readest has other sync paths for richer app state, but they are outside the
core repo. The no-WebDAV pilot intentionally does not sync Readest highlights,
notes, bookmarks, collections, or ratings.

## Current Watchlist

Issue and forum review found recent failures around KOSync and OPDS. Most were
closed or tied to non-official sync endpoints, but they define the tests we
must run before trusting the system:

- OPDS browsing can work while downloads fail on iOS/macOS, especially with
  older or password-protected servers.
- Calibre metadata writes, send-to-device workflows, EPUB-to-KEPUB conversion,
  and alternate imports can change the file identity.
- Some KOReader-compatible server implementations return progress in ways
  Readest cannot resolve accurately. The pilot should use official
  `koreader/kosync` first.
- Readest has had recent bugs where selecting remote progress did not move the
  reader or stale local progress overwrote the remote value. These are why the
  matrix tests lower/later progress and every important direction.
- The public default KOReader sync server has had outages. Self-hosting reduces
  that dependency but means we own Redis persistence, TLS, and health checks.
...
This is the main failure mode. The digest is a partial MD5/content hash, not a
full-file MD5. Filename matching is easier but too weak for editions, renamed
downloads, and duplicate titles.

## Candidate Matrix

| Candidate | Verdict | Notes |
|---|---:|---|
| Calibre + official KOSync + Readest OPDS | Accept | Best match for all required non-stock-Kobo devices with no paid/cloud progress service. |
...
| Readest WebDAV | Defer | Self-hosted and useful for Readest-only state, but it is a separate sync channel and should not ship in the default build. |
...
- Make onboarding idempotently reconcile the registry into Calibre, KOSync, nginx maps, and setup pages.
...
- Backup/restore covers `/srv/books/library`, `/srv/books/kosync`, and any sidecar databases.
...
- updated backup/restore docs for KOSync Redis
