...
test("KOReader starter bundles include account settings and SimpleUI paths", () => {
  ...
  assert.match(patch, /registerPatchPluginFunc\("coverbrowser"/);
  assert.match(patch, /getPluginInstance\("coverbrowser"\)/);
  assert.match(patch, /coverbrowser:setDisplayMode\("mosaic_image"\)/);
  assert.match(patch, /apply_cover_default/);
  assert.match(patch, /mosaic_image/);
  assert.doesNotMatch(patch, /bookinfomanager/);
