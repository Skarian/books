...
test("KOReader starter bundles include account settings and SimpleUI paths", () => {
  ...
  assert.match(patch, /if marker == nil then/);
  assert.match(patch, /plugins\/coverbrowser\.koplugin\/\?\.lua/);
  assert.match(patch, /pcall\(require, "bookinfomanager"\)/);
  assert.match(patch, /bim:saveSetting\("filemanager_display_mode", "mosaic_image"\)/);
  assert.match(patch, /bim:saveSetting\("history_display_mode", "mosaic_image"\)/);
  assert.match(patch, /bim:saveSetting\("collection_display_mode", "mosaic_image"\)/);
  assert.match(patch, /bim:closeDbConnection\(\)/);
  assert.match(patch, /mosaic_image/);
  assert.doesNotMatch(patch, /registerPatchPluginFunc\("coverbrowser"/);
  assert.doesNotMatch(patch, /coverbrowser:setDisplayMode/);
