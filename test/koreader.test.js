...
test("KOReader starter bundles include account settings and SimpleUI paths", () => {
  ...
  assert.ok(android.includes("koreader/plugins/books_status.lua"));
  ...
  assert.ok(kobo.includes(".adds/koreader/patches/2-kobo-style-screensaver.lua"));
  assert.ok(!android.includes("koreader/patches/2-kobo-style-screensaver.lua"));
  ...
  assert.ok(kobo.includes(".adds/koreader/plugins/books.koplugin/requests.lua"));
  assert.ok(kobo.includes(".adds/koreader/plugins/books_status.lua"));
  ...
  assert.match(booksConfig, /https:\/\/books\.test\/requests/);
  ...
  const bookRequests = zipRead(androidBundle.path, "koreader/plugins/books.koplugin/requests.lua");
  const bookStatus = zipRead(androidBundle.path, "koreader/plugins/books_status.lua");
  ...
  assert.match(booksPlugin, /Requests/);
  assert.match(booksPlugin, /registerToMainMenu\(self\)/);
  assert.match(booksPlugin, /sorting_hint = "tools"/);
  assert.match(booksPlugin, /\{ "sui_settings", "history", sync_action, request_action, "power" \}/);
  assert.match(booksPlugin, /Config\.saveTabConfig\{ "homescreen", "home", group \}/);
  assert.match(booksPlugin, /books_simpleui_seeded_v3/);
  ...
  assert.match(bookRequests, /Searching…/);
  assert.match(bookRequests, /Submitting…/);
  assert.match(bookRequests, /title = _\("Request\?"\)/);
  assert.match(bookRequests, /text = _\("Submit"\)/);
  assert.match(bookRequests, /Submitted successfully!/);
  assert.match(bookRequests, /_\("Error: "\)/);
  assert.doesNotMatch(bookRequests, /Search failed:|Could not request this book:/);
  assert.match(bookRequests, /ResultsDialog = InputContainer:extend/);
  assert.match(bookRequests, /MovableContainer:new/);
  assert.match(bookRequests, /TitleBar:new/);
  assert.match(bookRequests, /ButtonTable:new/);
  assert.match(bookRequests, /"◁◁", "▷▷"/);
  assert.match(bookRequests, /New Search/);
  assert.match(bookRequests, /pcall\(rapidjson\.decode, response\)/);
  assert.match(bookRequests, /type\(response\.results\) ~= "table"/);
  assert.match(bookRequests, /err == "already_in_library"/);
  assert.match(bookRequests, /invalid_response/);
  assert.doesNotMatch(bookRequests, /sui_window|SUIWindow/);
  assert.match(booksPlugin, /function Books:addToMainMenu/);
  assert.match(bookRequests, /cover_url/);
  assert.match(bookRequests, /★ /);
  assert.match(bookStatus, /InfoMessage:new/);
  assert.match(bookStatus, /UIManager:forceRePaint/);
  assert.match(bookStatus, /table\.pack/);
  assert.match(bookStatus, /unpack\(result, 1, result\.n\)/);
  ...
  assert.match(koboPatch, /books_kobo_style_screensaver_v1/);
  assert.match(koboPatch, /current_type == nil or current_type == "cover"/);
  assert.match(koboPatch, /"kobo_style_show_cover"\] = true/);
  assert.match(koboPatch, /"kobo_style_show_title"\] = true/);
  assert.match(koboPatch, /"kobo_style_show_chapter"\] = true/);
  ...
  assert.match(koboStylePatch, /self\.screensaver_type ~= "kobo_style"/);
  assert.match(koboStylePatch, /kobo_style_show_quote/);
...
test("KOReader starter bundles include AI dictionary plugin only when AI is enabled", () => {
  ...
  assert.match(plugin, /Status\.run/);
