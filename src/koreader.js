...
function writeLegacyKosyncPatch(file, settings, network, token) {
  ...
  fs.writeFileSync(file, [
    ...
    "end",
    "if marker == nil then",
    "  local apply_cover_default = true",
    '  require("userpatch").registerPatchPluginFunc("coverbrowser", function()',
    "    if not apply_cover_default then return end",
    "    apply_cover_default = false",
    '    local coverbrowser = require("pluginloader"):getPluginInstance("coverbrowser")',
    '    if coverbrowser then coverbrowser:setDisplayMode("mosaic_image") end',
    "  end)",
    "end",
